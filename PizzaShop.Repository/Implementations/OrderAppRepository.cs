...
public class OrderAppRepository : IOrderAppRepository
{
        ...
        List<ModifiersGroupForMenuViewModel>? result = await _dbo.Itemmodifiergroupmappings
            ...
            .Select(img => new ModifiersGroupForMenuViewModel
            {
                ...
                Modifiers = img.Modifiergroup.Itemmodifiergroupmappings.Where(img => img.Isitemmodifiable == true).Select(mgm => new ModifiersItemForMenuViewModel
                {
                    ...
                    ItemType = _dbo.Items.Where(i => i.Itemid == itemId).FirstOrDefault().Itemtype,
                    IsSelected = false
                }).ToList()
            }).ToListAsync();
    ...
    public async Task<string?> GetItemInstructionAsync(int orderDetailId)
    {
        try
        {
            string? instruction = _dbo.Orderdetails.FirstOrDefault(o => o.Orderdetailid == orderDetailId).Iteminstruction;
            return instruction;
        }
        catch (Exception ex)
        {
    ...
    public async Task<bool> SaveItemInstructionAsync(int orderDetailId, string instruction)
    {
        try
        {
            Orderdetail? orderDetail = await _dbo.Orderdetails.FirstOrDefaultAsync(o => o.Itemid == orderDetailId);
            if (orderDetail == null)
                return false;
    ...
    public async Task<List<Orderdetail>> SaveOrderItemsAsync(SaveOrderRequestViewModel model)
    {
        ...
        try
        {
            ...
            foreach (Orderdetail? item in itemsToDelete)
            {
                ...
                {
                    // Skip deletion — you could log it or handle it based on your need
                    // throw new InvalidOperationException($"Cannot delete item '{item.Itemname}' as it is already prepared.");
                }
            ...
            foreach (OrderItemForRowViewModel? item in model.OrderItems)
            {
                ...
                if (existingItem == null)
                {
                    ...
                    foreach (ModifierForMenuOrderViewModel? modifier in item.SelectedModifiers)
                    {
                        ...
                        {
                            Orderdetailid = orderDetail.Itemid,
