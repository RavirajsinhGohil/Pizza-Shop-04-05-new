...
async function OpenEditCustomerModal(orderId) {
    ...
    if (!r.ok) { alert('Could not load customer'); return; }
...
//itemWise Comments
function openItemInstructionModal(orderDetailId) {
    $('#ItemDetailId').val(orderDetailId);
    ...
    //     },
    //     error: function () {
    //         alert('Failed to load instruction.');
    //     }
    // });
}
...
//For Save Order
function saveOrder() {
    var orderItems = []; // Collect items from your JavaScript dynamically
    var selectedTaxes = []; // Collect checked tax ids
    ...
    // Loop through your order rows and create the orderItems array
    $(".items").each(function (index) {
        var itemId = $(this).data("item-id");  // Ensure item ID is stored in data-item-id
        console.log("Item ID:", itemId);

        var itemName = $(this).find(".item-name").text();  // Get item name
        console.log("Item Name:", itemName);

        var quantity = parseInt($(this).find(".qty-input").val());  // Get quantity
        console.log("Quantity:", quantity);

        var rate = parseFloat($(this).find(".cart-row").data("price"));  // Get rate from data-price
        console.log("Rate:", rate);

        // var instruction = $(this).find(".item-instruction").val();  // Get item instruction (if any)
        // console.log("Instruction:", instruction);

        var selectedModifiers = [];

        // Loop through any selected modifiers for the item
        $(this).find(".modifier").each(function () {
            selectedModifiers.push({
                modifierId: $(this).data("modifier-id"),  // Ensure modifier ID is stored in data-modifier-id
                modifierName: $(this).data("modifier-name"),  // Get modifier name
                rate: parseFloat($(this).data("modifier-price"))  // Get modifier price
            });
        });
    ...
    // Collect selected tax ids (checkboxes)
    $(".tax-checkbox:checked").each(function () {
        selectedTaxes.push(parseInt($(this).val()));  // Collect tax IDs
    });

    console.log("Selected Taxes:", selectedTaxes);

    // Make AJAX call to save order data
    $.ajax({
        type: "POST",
        ...
        success: function (response) {
            ...
            if (xhr.responseText) {
                toastr.error(xhr.responseText);  // Show server message like "Cannot delete item..."
            } else {
    ...
    $.ajax({
        ...
        success: function (orderItems) {
            ...
            Promise.all(renderPromises).then(function () {
                updateTotals(); // Call after all items are rendered
            });
        },
        error: function () {
