...
public class OrderAppController : Controller
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetItemInstruction(int id)
    {
        string? instruction = await _orderAppService.GetItemInstructionAsync(id);
        instruction ??= "";
    ...
    public async Task<IActionResult> SaveItemInstruction(int ItemDetailId, string instruction)
    {
        if (string.IsNullOrWhiteSpace(instruction))
            return BadRequest("Instruction is required.");

        bool success = await _orderAppService.SaveItemInstructionAsync(ItemDetailId, instruction);
        if (!success)
            return NotFound();
